const canvas = document.getElementById('2d-canvas');
        ...
        function showEndpoints(wall) {
            endpoint1.style.left = `${wall.x1 - 5}px`;
            endpoint1.style.top = `${wall.y1 - 5}px`;
            endpoint1.style.display = 'block'; // Show start endpoint

            endpoint2.style.left = `${wall.x2 - 5}px`;
            endpoint2.style.top = `${wall.y2 - 5}px`;
            endpoint2.style.display = 'block'; // Show end endpoint

            // Handle dragging of endpoints
        ...
        canvas.addEventListener('mousemove', (event) => {
            ...
            if (resizing && selectedWall) {
                // Resize the wall based on the selected endpoint
                if (resizingPoint === 'start') {
                    selectedWall.x1 = x; // Update the start point
                    selectedWall.y1 = y; // Allow vertical adjustment
                } else if (resizingPoint === 'end') {
                    selectedWall.x2 = x; // Update the end point
                    selectedWall.y2 = y; // Allow vertical adjustment
                }

                // Clear and redraw
                ctx.clearRect(0, 0, canvas.width, canvas.height);
                drawGrid();
                drawWalls(); // Redraw walls without extra space
                showEndpoints(selectedWall); // Show updated endpoints
                return; // Exit to prevent further processing
            }

            const isHoveringOnWall = walls.some(wall => isPointOnLine(x, y, wall));
            canvas.style.cursor = isHoveringOnWall ? 'pointer' : 'crosshair'; // Change cursor on hover

            if (isDragging && selectedWall) {
                const dx = x - selectedWall.x1; // Calculate delta
                const dy = y - selectedWall.y1;

                // Update the wall's coordinates
                selectedWall.x1 += dx;
                selectedWall.y1 += dy;
                selectedWall.x2 += dx;
                selectedWall.y2 += dy;
                ...
                drawEnds(); // Redraw the filled circles
                // Hide endpoints when dragging the wall
        ...
        canvas.addEventListener('mouseup', (event) => {
            if (drawing) {
                ...
                walls.push({ x1: currentLine.x1, y1: currentLine.y1, x2: currentLine.x2, y2: currentLine.y2, length, texture: wallTexture, highlighted: false }); // Store length with wall data
                ...
                deleteButton.style.display = 'none'; // Hide delete button after drawing
            }
        ...
        function showDeleteButton(x, y) {
            ...
            deleteButton.style.top = `${y - 20}px`; // Adjust position to be above the wall
